...
for (s in subexpts) {
  ...
  n_replicates <- s %>% str_extract_all("\\)\\d+|\\d+\\(") %>% unlist() %>%
    .[1] %>% str_replace_all("\\(|\\)", "")

  if (is.na(n_replicates)) { n_replicates <- 1 }

  # Extract the multiplicands
  contents <- s %>% str_extract_all("\\([^\\(\\)]*\\)") %>% unlist() %>%
  str_replace_all("\\(|\\)", "")
  ...
  for (i in contents) {
    ...
    elements <- i %>% str_split(";") %>% unlist() %>%
    lapply(function(x) unlist(str_split(x, ","))) %>% lapply(trimws)
